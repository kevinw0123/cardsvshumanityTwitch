    ...
    if(message !== null){
        ...
        if(parsed !== null){
            ...
            if (parsed.message.includes("!cards ")) {
                var cardsCommand = parsed.message.substr(6, parsed.message.length);
                if (cardsCommand.includes("start")) {
                    console.log("CARDS IS STARTING");

                } else if (cardsCommand.message.includes("end")) {
                    console.log("CARDS IS ENDING");
                }
            }
...
// source = http://stackoverflow.com/questions/19440589/parsing-json-data-from-a-url
