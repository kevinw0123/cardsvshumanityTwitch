...
var chatClient = function chatClient(options){
    ...
    this.numBlackCards = cardDeck.Base.black.length;
    this.numWhiteCards = cardDeck.Base.white.length;
...
var roundObject = function roundObject(roundInfo) {
    this.currVoters = {};
    this.blackCard  = roundInfo.blackCard;
    this.whiteCards = [];
    for (var i=0; i<6; i++) {
        this.whiteCards[i] = roundInfo.whiteCards[i];
    }
    this.picks      = roundInfo.picks;
    this.voteOrder = "";
    
}

chatClient.prototype.newGame = function newGame() {
    this.gameObject = new gameObject();
}

chatClient.prototype.startRound = function startRound() {
    //draw cards for the current round
    var blackIndex      = Math.floor(Math.random()*this.numBlackCards);
    var tempBlackCard   = cardDeck.blackCards[blackIndex].text;
    var tempPicks       = cardDeck.blackCards[blackIndex].pick;
    tempWhiteCards      = [];
    for (var i=0; i<6; i++) {
        tempWhiteCards[i] = cardDeck.whiteCards[Math.floor(Math.random()*this.numWhiteCards)];
    }

    //start round
    this.round          = this.round + 1;
    this.gameObject.startedRound   = true;
    //create roundObject
    this.gameObject.roundObject  = new roundObject({
        blackCard:  tempBlackCard,
        whiteCards: tempWhiteCards,
        picks:      tempPicks,
    });
    //populate html
    $('.blackcard').text(tempBlackCard);
    for (var i=1; i<=6; i++) {
        $('#whitecard'+i).text(tempWhiteCards[i-1]);
    }
    ...
    if (socket !== null && socket.readyState === 1) {
      var msgBuffer = 'PRIVMSG ' + this.channel + ' : Q:  ' + tempBlackCard + '\r\n'
      socket.send(msgBuffer);
      
      var ansBuffer = 'PRIVMSG ' + this.channel + ' : A:  ';
      for (var i=1; i<=6; i++) {
        ansBuffer = ansBuffer + i + '. ' + tempWhiteCards[i-1] + '\n  ';
      }
      ansBuffer = ansBuffer + '\r\n';
      socket.send(ansBuffer);
    }
}

chatClient.prototype.calculateScore = function calculateScore() {
    for (var key in this.gameObject.roundObject.currVoters) {
        if (this.gameObject.roundObject.currVoters[key] == this.gameObject.roundObject.voteOrder) {
            if (this.gameObject.highscore[key] == null || this.gameObject.highscore[key] == 0) {
                this.gameObject.highscore[key] = 10;
            } else {
                this.gameObject.highscore[key] = this.gameObject.highscore[key] + 10;
            }
        } else {
            if (this.gameObject.highscore[key] == null) {
...
chatClient.prototype.setOrder = function setOrder(voteOrder) {
    // if the number of picks left > the num chosen
    
    if (this.gameObject.roundObject.picks > this.gameObject.roundObject.voteOrder.length) {
        // if a card has been chosen already, replace it at the end
        if (this.gameObject.roundObject.voteOrder.includes(voteOrder)) {
            this.gameObject.roundObject.voteOrder = this.gameObject.roundObject.voteOrder.replace(voteOrder, "");
        }
        this.gameObject.roundObject.voteOrder = this.gameObject.roundObject.voteOrder + voteOrder;
    } else if (this.gameObject.roundObject.picks <= 1) {
        this.gameObject.roundObject.voteOrder = voteOrder;
    }
    if (this.gameObject.roundObject.picks == this.gameObject.roundObject.voteOrder.length) {
      $('.submit').fadeIn(500);
    }
...
chatClient.prototype.onMessage = function onMessage(message){
    if(message !== null){
        if (this.gameObject.startedRound == true) {
            var parsed = this.parseMessage(message.data);
            if (parsed != null) {

                var vote = parsed.message.trim().replace(" ", "");
                if (vote.includes("#vote")) {
                    vote = vote.substring(5,vote.length);
                    this.gameObject.roundObject.currVoters[parsed.username] = vote;
                  
                }
                //test
                var currvotersbuffer = "";
                var size = 0;
                for (var key in this.gameObject.roundObject.currVoters) {
                  currvotersbuffer = currvotersbuffer + key + " ";
                  currvotersbuffer = currvotersbuffer + this.gameObject.roundObject.currVoters[key] + " ";
                  size++;
                }
                $('.votesin').text("Votes: " + size);
                console.log("Curr voters: " + currvotersbuffer);
                //end test
            }    
        }
    }
...
/************************************************************************/
chatClient.prototype.buildLeaderboard = function buildLeaderboard(){
    ...
    //     });
    var size = 0, key;
    console.log(this.gameObject.highscore);
    for (var key in this.gameObject.highscore) {
        if (this.gameObject.highscore.hasOwnProperty(key)) size++;
    }
    
    console.log(size);

    var highscorearr = this.gameObject.highscore;

    var chatKeys = Object.keys(highscorearr),
        outputTemplate = $('#entry-template').html(),
        leaderboard = $('.leaderboard-output'),
        sortedData = chatKeys.sort(function(a,b){
            return highscorearr[b] - highscorearr[a];
        });
    leaderboard.empty();

    var leaderheight = Math.min(10, sortedData.length);
  
    for(var i = 0; i < leaderheight; i++){
        var viewerName = sortedData[i],
            template = $(outputTemplate);

//        template.find('.rank').text(i + 1);
        template.find('.user-name').text(viewerName);
        template.find('.user-points').text(this.gameObject.highscore[viewerName] +" points"); 
        leaderboard.append(template);
        console.log("after appending " + viewerName);
    }
}