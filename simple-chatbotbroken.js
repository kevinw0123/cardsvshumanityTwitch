...
var roundObject = function roundObject(roundOptions) {

	this.blackCard = roundOptions.blackCard;
	this.whiteCards = [];
...
chatClient.prototype.onMessage = function onMessage(message){
    if(message !== null){
        ...
        if(parsed !== null){
            ...
            if (parsed.message.includes("!cards ")) {
                var cardsCommand = parsed.message.substr(7, parsed.message.length);
                if (cardsCommand.includes("start")) {
                	if (roundStarted == true) return;
                	roundStarted = true;

                    console.log("shuffling cards");
                    //load deck
                    var blackLen = carddeck.Base.black.length;
                    var whiteLen = carddeck.Base.white.length;
                    var currBlack = Math.floor(Math.random()*blackLen);
                    console.log(carddeck.blackCards[currBlack].text);


                    for (var i=0; i<6; i= i+1) {
                    	whiteCardArray[i] = 0;
						console.log((i+1) + ": " + carddeck.whiteCards[Math.floor(Math.random()*whiteLen)]);	
                    }


                    //how many cards to pick
                    if (carddeck.blackCards[currBlack].pick != 1) {
	                    console.log("Choose " + carddeck.blackCards[currBlack].pick + " cards.");	
                    } else {
                    	console.log("Choose " + carddeck.blackCards[currBlack].pick + " card.");	
                    }

                } else if (cardsCommand.includes("end")) {
                	if (roundStarted == false) return;
                	roundStarted == false;
                    console.log("game over");
                } else {
                	if (roundStarted == false) return;
                	
                	var vote = cardsCommand.substr(0,1);
                	cardsCommand.substr(2,cardsCommand.length);
                	console.log("|"+ vote + "|");
                	
                	// while (cardsCommand.length >= 1) {
                	// 	console.log("Voted for card " + vote);
	                // 	var vote = cardsCommand.substr(0,1);
	                // 	cardsCommand.substr(1,cardsCommand.length);
                	// }
                	
                } 
            }

            // userPoints = sessionStorage.getItem(parsed.username);
            // if(userPoints === null){
            //     sessionStorage.setItem(parsed.username, null);
        ...
        // if (parsed !== null) {
        //     localStorage.setItem(parsed.username, 0);   
        // }
        // if (parsed == "!cardsagainsthumanity") {
        //     userPoints = localStorage.getItem(parsed.username);
        //     if (userPoints === null) {
        //         localStorage.setItem(parsed.username, 1);
        //     } else {
        //         localStorage.setItem(parsed.username, parseFloat(userPoints) + 1);
        //     }
        // }   
    }
};
...
function buildLeaderboard(){
    ...
    for(var i = 0; i < 10; i++){
        ...
        template.find('.user-points').text(sessionStorage[viewerName]);
